...
impl ModulationSource for LfoModulationSource {
    ...
    fn process(&mut self, output: &mut [f32]) {
        // Process LFO for each sample in block
        for sample in output {
            *sample = self.lfo.run() as f32;
        }
    }
}
...
impl ModulationSource for AhdsrModulationSource {
    ...
    fn process(&mut self, output: &mut [f32]) {
        // Use AhdsrEnvelope's optimized process_buffer method
        self.envelope.process_buffer(&self.parameters, output);
    }
}
...
pub struct ModulationMatrix {
    ...
    /// Current block size (may be less than MAX_BLOCK_SIZE at event boundaries but never more)
...
impl ModulationMatrix {
    ...
    /// Supports partial blocks for event boundaries (e.g., 37 samples instead of MAX_BLOCK_SIZE).
    ///
    /// # Arguments
    /// * `chunk_size` - Number of samples to process (1-MAX_BLOCK_SIZE, typically MAX_BLOCK_SIZE)
    pub fn process(&mut self, chunk_size: usize) {
        assert!(
            chunk_size <= MAX_MODULATION_BLOCK_SIZE,
            "Chunk size exceeds maximum block size"
        );
        ...
        // Memorize valid block size
        self.current_block_size = chunk_size;
    }

    /// Get accumulated preprocessed modulation values for a parameter for the entire current block.
    ///
    /// Writes the sum of all modulation sources targeting the given parameter to the output buffer.
    /// The output buffer must be at least `current_block_size` long.
    pub fn get_modulation_block(&self, parameter_id: FourCC, output: &mut [f32]) {
        let block_size = self.current_block_size;
        debug_assert!(
            ...
            if slot.enabled {
                for target in &slot.targets {
                    if target.parameter_id == parameter_id {
                        ...
                        for (out, &inp) in output[..block_size]
                            .iter_mut()
                            .zip(&slot.block_buffer[..block_size])
                        {
                            *out += inp * amount;
                        }
                    }
    ...
    #[inline]
    pub fn get_modulation(&self, parameter_id: FourCC, sample_index: usize) -> f32 {
        debug_assert!(
            sample_index < self.current_block_size,
        ...
        for slot in &self.lfo_slots {
            if slot.enabled {
                for target in &slot.targets {
                    if target.parameter_id == parameter_id {
                        let mod_value = slot.block_buffer[sample_index];
