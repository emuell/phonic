...
pub struct GranularParameters {
    /// Grain window mode.
    pub grain_window: GrainWindowMode,
...
impl Default for GranularParameters {
    fn default() -> Self {
        Self {
            grain_window: GrainWindowMode::Triangle,
            grain_size: 10.0,
            grain_density: 10.0,
            grain_spray: 0.0,
            grain_variation: 0.0,
...
impl GranularParameters {
    ...
    pub fn validate(&self) -> Result<(), Error> {
        if self.grain_size < 1.0 || self.grain_size > 1000.0 {
            return Err(Error::ParameterError(
                "Grain size must be between 1 and 1000 ms".to_string(),
            ));
        }

        if self.grain_density < 1.0 || self.grain_density > 100.0 {
            return Err(Error::ParameterError(
                "Grain density must be between 1.0 and 100.0 Hz".to_string(),
            ));
        }

        if self.grain_spray < 0.0 || self.grain_spray > 1.0 {
            return Err(Error::ParameterError(
                "Grain spray must be between 0.0 and 1.0".to_string(),
            ));
        }

        if self.grain_variation < 0.0 || self.grain_variation > 1.0 {
            return Err(Error::ParameterError(
                "Grain variation must be between 0.0 and 1.0".to_string(),
            ));
        }

        if self.grain_pan_spread < 0.0 || self.grain_pan_spread > 1.0 {
            return Err(Error::ParameterError(
...
impl<const POOL_SIZE: usize> GrainPool<POOL_SIZE> {
    ...
    pub fn playback_position(&self, parameters: &GranularParameters, position_mod: f32) -> f32 {
        ...
        // Apply modulation
        base_position += position_mod * 0.5;
    ...
    fn try_trigger_grain(
        ...
        let spray_variation = if !self.sample_buffer.is_empty() {
            ...
            let spray_seconds =
                parameters.grain_spray as f64 * 4.0 * (self.rng.random::<f64>() - 0.5);
        ...
        // Start a new grain
        self.activate_new_grain(parameters, grain_position, size_mod)
    }

    /// Advance the playhead position for PlayThrough mode.
    #[inline]
    fn advance_playhead(&mut self, buffer_frame_count: usize, playhead_speed: f32) {
        // Advance position by one frame worth of time at the current playback speed
        let position_increment = playhead_speed / buffer_frame_count as f32;
        ...
        match channel_count {
            1 => {
                ...
                for (frame_index, frame) in output.as_frames_mut::<1>().iter_mut().enumerate() {
                    ...
                    // Move Playhead
                    if move_playhead {
                        self.advance_playhead(sample_frame_count, parameters.playhead_speed);
                    }
    ...
    fn update_trigger_phase(
        ...
        let density = (granular_params.grain_density * density_mult).clamp(1.0, 100.0);
    ...
    /// Returns true if a grain was successfully activated, false if no free grains available.
    fn activate_new_grain(
        ...
        if let Some(index) = self.grain_pool.iter().position(|g| !g.is_active()) {
            ...
            let window_mode = parameters.grain_window;
            let speed = self.speed;

            // Calculate variation
            let variation = parameters.grain_variation;
            ...
            let grain_size_ms = (parameters.grain_size * size_mult).clamp(1.0, 1000.0);

            let grain_size =
                ((grain_size_ms * size_scale * self.sample_rate as f32 / 1000.0) as usize).max(2);

            let panning_spread =
                parameters.grain_pan_spread * (self.rng.random::<f32>() * 2.0 - 1.0);
