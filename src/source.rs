...
use crate::source::resampler::{AudioResampler, ResamplingQuality, ResamplingSpec};
...
pub struct ChannelMappedSource<S> {
    source: S,
...
impl<S> AudioSource for ChannelMappedSource<S>
where
    S: AudioSource,
{
    fn write(&mut self, output: &mut [f32]) -> usize {
