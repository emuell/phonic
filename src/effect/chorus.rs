...
impl ChorusEffect {
    ...
    pub fn new() -> Self {
        Self {
            ...
            wet_mix: Self::DEFAULT_WET_MIX,
            filter_type: Self::DEFAULT_FILTER_TYPE,
            filter_freq: Self::DEFAULT_FILTER_FREQ,
            filter_resonance: Self::DEFAULT_FILTER_RESONANCE,
...
impl Effect for ChorusEffect {
    ...
    fn process_message(&mut self, message: &EffectMessage) {
        if let Some(message) = message.downcast_ref::<ChorusEffectMessage>() {
            match message {
            ...
            panic!("ChorusEffect: Received invalid/unknown message '{message:?}'");
        }
    }
}
