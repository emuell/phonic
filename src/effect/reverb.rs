...
/// A reverb effect ported from [Airwindows' "Reverb" plugin](https://www.airwindows.com/reverb/).
...
impl ReverbEffect {
    const DEFAULT_ROOM_SIZE: f64 = 0.6;
    const DEFAULT_WET: f64 = 0.5;
...
impl Effect for ReverbEffect {
    ...
    fn process_message(&mut self, message: &EffectMessage) {
        if let Some(message) = message.downcast_ref::<ReverbEffectMessage>() {
            match message {
                ReverbEffectMessage::Init(room_size, wet) => {
                    self.set_room_size((*room_size).clamp(0.0, 1.0) as f64);
                    self.set_wet(*wet as f64);
                }
            ...
            panic!("ReverbEffect: Received invalid/unknown message '{message:?}'");
        }
    }
}
