...
use crate::{
    ...
    source::{
        ...
        mixed::{EffectProcessor, MixedSource, MixerMessage, SubMixerProcessor},
        panned::PannedSource,
...
use crate::source::mixed::SubMixerThreadPool;
...
pub struct PlayerConfig {
    /// Whether parallel mixer processing is enabled.
    ///
    /// Even when enabled, the player will automatically fall back to sequential processing
    /// when the thresholds are not met, avoiding unnecessary overhead for simple mixer graphs.
    ///
    /// Default: `true` (enabled)
    pub parallel_processing_enabled: bool,

    /// Number of parallel mixer processing worker threads to spawn.
    ///
    /// `None` means auto-detect based on available CPU cores.
    /// Default: `None` (auto)
    pub parallel_processing_thread_count: Option<usize>,
}
...
impl PlayerConfig {
    ...
    pub fn new() -> Self {
        Self {
            ...
            parallel_processing_thread_count: None,
        }
    }
    ...
    pub fn parallel_processing_thread_count(mut self, count: usize) -> Self {
        self.parallel_processing_thread_count = Some(count);
        self
    }

    /// Set parallel mixer thread count.
    pub fn parallel_processing_effective_thread_count(&self) -> usize {
        self.parallel_processing_thread_count.unwrap_or_else(|| {
            // Use number of physical cores, or threads - 2 for a bit of headroom for file I/O
            num_cpus::get_physical().max(num_cpus::get().saturating_sub(2))
        })
    }
}
...
impl Player {
    ...
    pub fn new_with_config<S: Into<Option<SyncSender<PlaybackStatusEvent>>>>(
        ...
        // Create thread pool for parallel mixers
        let thread_pool = config.parallel_processing_enabled.then(|| {
            Arc::new(SubMixerThreadPool::new(
                config.parallel_processing_effective_thread_count(),
                output_device.sample_rate(),
            ))
        });
        main_mixer.set_parallel_processing(thread_pool);
