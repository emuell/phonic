//! An example for testing and showcasing parallel mixer processing.
//!
//! Usage:
//!   cargo run --release --example play-parallel --features fundsp -- [OPTIONS]
//!
//! Options:
...
//!   --parallel <true|false>  Enable parallel processing (default: true)
...
use phonic::{
    ...
    utils::{ahdsr::AhdsrParameters, fundsp::shared_ahdsr, speed_from_note},
    DefaultOutputDevice, Error, FilePlaybackOptions, GeneratorPlaybackOptions, MixerId, Parameter,
    Player, PlayerConfig, SynthPlaybackOptions,
};
...
fn main() -> Result<(), Error> {
    ...
    let parallel_enabled = args.parallel.unwrap_or(true);
    ...
    let mut config = PlayerConfig::new().parallel_processing(parallel_enabled);
    if let Some(thread_count) = args.threads {
        config = config.parallel_processing_thread_count(thread_count);
    }
    ...
    let mut submixers: Vec<MixerId> = Vec::new();
    ...
    for mixer_idx in 0..submixer_count {
        ...
        for source_idx in 0..sources_per_mixer {
            let source_type = rand::random_range(0..7);
            let frequency = 110.0 * 2.0_f32.powi(rand::random_range(0..4));
            ...
            match source_type {
                ...
                4 => {
                    ...
                    player.play_file(
                        ...
                        FilePlaybackOptions::default()
                            .streamed()
                            .volume_db(-12.0)
                            .speed(speed_from_note(rand::random_range(48..72)))
                            .target_mixer(mixer.id())
                ...
                5 => {
                    ...
                    for i in 0..4 {
                        let note = rand::random_range(36..60);
                ...
                _ => {
                    ...
                    let now = player.output_sample_frame_position();
                    for i in 0..3 {
                        let note = rand::random_range(48..72);
        ...
        for _effect_idx in 0..effects_per_mixer {
            let effect_type = rand::random_range(0..3);

            match effect_type {
                0 => {
                    let gain_db = rand::random_range(-6.0..0.0);
                    player.add_effect(GainEffect::with_gain_db(gain_db), mixer.id())?;
                }
                1 => {
                    let wet = rand::random_range(0.2..0.5);
                    let decay = rand::random_range(0.3..0.7);
    ...
    println!("Parallel processing: {parallel_enabled}");
    let effective_thread_count = config.parallel_processing_effective_thread_count();
