...
const backend = {
    ...
    getEffectParameterString(effectId, paramId, normalizedValue) {
        const valuePtr = ccall('get_effect_parameter_string', 'number', ['number', 'number', 'number'],
    ...
    setEffectParameterValue(effectId, paramId, normalizedValue) {
        ccall('set_effect_parameter_value', null, ['number', 'number', 'number'],
            [effectId, paramId, normalizedValue]);
    }
};
...
class EffectManager {
    ...
    createParameterControl(effectId, param) {
        ...
        if (param.type === 'Float') {
            ...
            let default_index = Math.floor(param.default * (param.values.length - 1))
